...
public final class DishListDemoWidget extends DocWidget<DishListDemoWidget.Params,
    ...
    protected List<String> bodyJs() {
        return List.of(
                ...
                "                     + 'save/restore round-trips the arranged view · headers are optional, freezable '",
                "                     + 'and can lead a copy · '",
                "                     + 'sort while editing to watch D7 defer the remap · click a header to sort, again "
              + "to reverse, a third time to clear · pin (\\uD83D\\uDCCC) a sorted column to keep it while "
              + "sorting another (multi-key, no modifier) · Alt+\\u2191/\\u2193 sort the cursor column, "
              + "Alt+Shift+\\u2191 pins it';",
                ...
                "    var STYLES = relation.styles;   // one declaration: the cell's options AND the sort order",
                "",
                ...
                "    // ext6 — style sorts in its DECLARED order (Chinese, French, English, ...),",
                "    // not alphabetically: the comparator comes with the Relation's meta.",
                "    btn('sort style \\u25B2', function () { grid.sortBy('style', 'asc'); });",
                "    btn('sort style \\u25BC', function () { grid.sortBy('style', 'desc'); });",
                "    // ext6 multi-key, no modifier: PIN a sorted column and it survives the next",
                "    // sort — pin style, then sort price, and prices order WITHIN each style.",
                "    btn('pin style',   function () { grid.sortBy('style', 'asc'); grid.pinSortKey('style', true); });",
                "    btn('unpin style', function () { grid.pinSortKey('style', false); });",
