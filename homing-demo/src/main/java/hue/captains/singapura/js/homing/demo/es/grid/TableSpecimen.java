...
final class TableSpecimen {
    ...
    static List<String> bodyJs(String label, int rows, int cols, String band, String note) {
        var js = new ArrayList<String>(List.of(
                ...
                "    // ext6 — the ordinal sorts by VALUE (…, 99, 100, 101), not as text",
                "    // ('1', '10', '100', '101', …): the comparator comes with the Relation.",
                "    btn('sort # \u25BC',   function () { grid.sortBy('#', 'desc'); });",
                "    btn('sort by region', function () { grid.sortBy('region', 'asc'); });",
                "    // ext6 multi-key: pin region, then 'sort #' orders the ordinal WITHIN each",
                "    // region — six regions over hundreds of rows are almost all ties.",
                "    btn('pin region',     function () { grid.sortBy('region', 'asc'); grid.pinSortKey('region', true); });",
                "    btn('unpin region',   function () { grid.pinSortKey('region', false); });",
